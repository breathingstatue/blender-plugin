...
part_prefixes = ['body', 'wheel', 'pin', 'axle', 'spring', 'spinner']
...
def assign_material_to_fin(scene):
    """Assign material to all imported objects for both COL and UV_TEX."""
    # Get all mesh objects in the scene
    mesh_objects = [obj for obj in scene.objects if obj.type == 'MESH']
    
    # Run texture assignment for both material choices
    set_material_to_fin_col(mesh_objects)
    set_material_to_fin_texture(mesh_objects)
...
def set_material_to_fin_texture(mesh_objects):
    """Sets the material to Texture (UV_TEX) for all mesh objects."""
    if not mesh_objects:
        print("No mesh objects selected for material assignment.")
        return
