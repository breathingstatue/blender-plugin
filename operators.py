...
class ExportRV(bpy.types.Operator):
	...
	# Dropdown menu for file formats with an update function
	format_type: bpy.props.EnumProperty(
		...
		update=lambda self, context: self.update_file_extension()
	)
	...
	def update_file_extension(self):
		...
		ext = ext_mapping.get(self.format_type, "")
...
def exec_export(filepath, format_type, context):
	...
	if frmt == 'PRM':
		from . import prm_out
		prm_out.export_file(filepath, context.scene, context)
